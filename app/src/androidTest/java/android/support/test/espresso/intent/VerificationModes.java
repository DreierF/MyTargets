...
public final class VerificationModes {
  ...
  private static final class Times implements VerificationMode {
    ...
    public void verify(Matcher<Intent> matcher, List<VerifiableIntent> recordedIntents) {
      ...
      if (matches != times) {
        fail(String.format("Wanted to match %d intents. Actually matched %d intents.\n\n"
            + "IntentMatcher: %s\n\nMatched intents:%s\n\nRecorded intents:%s", times,
            matches, matcher, joinOnDash(matchedIntents),
            joinOnDash(recordedIntents)));
      }
