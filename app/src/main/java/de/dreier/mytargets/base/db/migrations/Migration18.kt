...
object Migration18 : Migration(17, 18) {
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        // Arrow migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `Arrow`( " +
                ...
                "`thumbnail` BLOB " +
                ")")
        database.execSQL("INSERT INTO `Arrow` " +
        ...
        database.execSQL("CREATE TABLE IF NOT EXISTS `ArrowImage`( " +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                "`fileName` TEXT," +
                "`arrow` INTEGER," +
                "FOREIGN KEY(`arrow`) REFERENCES Arrow(`_id`) ON UPDATE NO ACTION ON DELETE CASCADE " +
                ")")
        database.execSQL("INSERT INTO `ArrowImage`(`fileName`,`arrow`) " +
                "SELECT `image`, `_id` " +
                "FROM ARROW_OLD " +
                "WHERE `image` <> ''")

        // Bow migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `Bow`( " +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                "`name` TEXT," +
                "`type` INTEGER," +
                "`brand` TEXT," +
                ...
                "`thumbnail` BLOB " +
                ")")
        database.execSQL("INSERT INTO `Bow` " +
                ...
                "FROM BOW_OLD")
        // Bow image migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `BowImage`(" +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                "`fileName` TEXT," +
                "`bow` INTEGER," +
                "FOREIGN KEY(`bow`) REFERENCES Bow(`_id`) ON UPDATE NO ACTION ON DELETE CASCADE " +
                ")")
        database.execSQL("INSERT INTO `BowImage`(`fileName`,`bow`) " +
                "SELECT `image`, `_id` " +
                "FROM BOW_OLD " +
                "WHERE `image` <> ''")
        // StandardRound migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `StandardRound`(" +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                "`club` INTEGER," +
                "`name` TEXT " +
                ")")
        database.execSQL("INSERT INTO `StandardRound` " +
                "SELECT `_id`,`club`,`name` " +
                "FROM STANDARD_ROUND_TEMPLATE_OLD " +
                "WHERE `club` < 512")
        // RoundTemplate migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `RoundTemplate`( " +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                ...
                ")")
        database.execSQL("INSERT INTO `RoundTemplate` " +
        ...
        database.execSQL("CREATE TABLE IF NOT EXISTS `Round`( " +
                ...
                ")")
        database.execSQL("INSERT INTO `Round` " +
        ...
        // End image migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `EndImage`( " +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                "`fileName` TEXT," +
                "`end` INTEGER," +
                "FOREIGN KEY(`end`) REFERENCES End(`_id`) ON UPDATE NO ACTION ON DELETE CASCADE" +
                ")")
        database.execSQL("INSERT INTO `EndImage`(`fileName`,`end`) " +
                "SELECT `image`, `_id` " +
                "FROM PASSE_OLD " +
                "WHERE `image` <> ''")
        ...
        // SightMark migration
        database.execSQL("CREATE TABLE IF NOT EXISTS `SightMark`( " +
                "`_id` INTEGER PRIMARY KEY AUTOINCREMENT," +
                "`bow` INTEGER," +
                "`distance` TEXT," +
                "`value` TEXT," +
                "FOREIGN KEY(`bow`) REFERENCES Bow(`_id`) ON UPDATE NO ACTION ON DELETE CASCADE " +
                ")")
        database.execSQL("INSERT INTO `SightMark` " +
