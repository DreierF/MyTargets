...
public class RelocationBatchResult {
    ...
    static class Serializer extends StructSerializer<RelocationBatchResult> {
        ...
        public void serialize(RelocationBatchResult value, JsonGenerator g, boolean collapse) throws IOException, JsonGenerationException {
            ...
            StoneSerializers.list(RelocationResult.Serializer.INSTANCE).serialize(value.entries, g);
            if (!collapse) {
                g.writeEndObject();
            }
        }
            ...
            if (tag == null) {
                ...
                while (p.getCurrentToken() == JsonToken.FIELD_NAME) {
                    ...
                    if ("entries".equals(field)) {
                        f_entries = StoneSerializers.list(RelocationResult.Serializer.INSTANCE).deserialize(p);
                    }
                    else {
                        skipValue(p);
                    }
